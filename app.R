              ...
              fluidRow(
                box(title = "Descarga de Datos", status = "primary", solidHeader = TRUE, width = 12,
                    div(style = "text-align: center;",
                        ...
                        p("Nota: La pestaña 'GES Avanzadas por Días' corresponde siempre a los datos del último corte disponible."),
                        br(),
...
server <- function(input, output, session) {
  ...
  # 9. DOWNLOAD - DATOS HISTÓRICOS (COMPLETOS) ----
  
  output$download_all <- downloadHandler(
    filename = function() {
      paste0(format(Sys.Date(), "%y%m%d"), "_ges_historico_completo.xlsx")
    },
    content = function(file) {
      # Usar los datos históricos que YA ESTÁN CARGADOS
      vigentes <- datos_historicos_vigentes
      retrasadas <- datos_historicos_retrasadas
      exceptuadas <- datos_historicos_exceptuadas
      
      # Crear pestaña de "Avanzadas" a partir de vigentes
      if("clasificacion_avance" %in% names(vigentes)) {
        avanzadas <- vigentes %>%
          filter(clasificacion_avance == "Avanzadas (Avance > 66% del plazo total)") %>%
          select(fecha_corte, problema_clasificado, responsable_de_garantia, es_oncologico)
        
        names(avanzadas) <- c("Fecha Corte", "Problema de Salud", "Responsable Garantía", "Oncológico")
      } else {
        avanzadas <- data.frame(
          "Fecha Corte" = character(),
          "Problema de Salud" = character(),
          "Responsable Garantía" = character(),
          "Oncológico" = logical()
        )
      }
      
      # Renombrar columnas de vigentes
      if("clasificacion_avance" %in% names(vigentes)) {
        names(vigentes) <- c("Fecha Corte", "Clasificación Avance", "Problema de Salud", "Responsable Garantía", "Oncológico")
      }
      
      if("tipo_retraso" %in% names(retrasadas)) {
        names(retrasadas) <- c("Fecha Corte", "Tipo Retraso", "Problema de Salud", "Responsable Garantía", "Oncológico")
      }
      
      if("periodo_excepcion" %in% names(exceptuadas)) {
      ...
      lista_datos <- list(
        ...
        "GES Avanzadas por Días" = avanzadas,
  ...
  # 10. DOWNLOAD - DATOS ÚLTIMO CORTE (FILTRADOS) ----
  ...
  output$download_filtered <- downloadHandler(
    ...
    content = function(file) {
      ...
      if(nrow(vigentes_filt) == 0 && nrow(retrasadas_filt) == 0 && nrow(exceptuadas_filt) == 0) {
        lista_datos <- list(
          "Sin datos" = data.frame(Mensaje = "No hay datos con los filtros seleccionados")
        )
        writexl::write_xlsx(lista_datos, path = file)
        return()
      }
      
      # Crear pestaña de "Avanzadas" a partir de vigentes filtrados
      if("clasificacion_avance" %in% names(vigentes_filt)) {
        avanzadas_filt <- vigentes_filt %>%
          filter(clasificacion_avance == "Avanzadas (Avance > 66% del plazo total)") %>%
          select(fecha_corte, problema_clasificado, responsable_de_garantia, es_oncologico)
        
        names(avanzadas_filt) <- c("Fecha Corte", "Problema de Salud", "Responsable Garantía", "Oncológico")
      } else {
        avanzadas_filt <- data.frame(
          "Fecha Corte" = character(),
          "Problema de Salud" = character(),
          "Responsable Garantía" = character(),
          "Oncológico" = logical()
      ...
      lista_datos <- list(
        ...
        "GES Avanzadas por Días" = avanzadas_filt,
