  ...
  dashboardBody(
    tabItems(
      tabItem(tabName = "retrasadas",
              ...
              fluidRow(
                box(title = "Top 10 Responsables con más Retrasos", status = "info", width = 6,
                    plotlyOutput("top_responsables_retrasadas", height = "450px")),
                box(title = "Top 10 Problemas de Salud con más Retrasos", status = "info", width = 6,
      ...
      tabItem(tabName = "vigentes",
              ...
              fluidRow(
                box(title = "Top 10 Responsables con más Vigentes", status = "info", width = 6,
                    plotlyOutput("top_responsables_vigentes", height = "450px")),
                box(title = "Top 10 Problemas de Salud Vigentes", status = "info", width = 6,
...
server <- function(input, output, session) {
  ...
  df_vigentes_raw <- read.csv(ruta_vigentes, stringsAsFactors = FALSE) %>%
    mutate(
      ...
      clasificacion_avance = case_when(
        porcentaje_avance <= 25 ~ "Tempranas (Avance ≤ 25% del plazo total)",
        porcentaje_avance <= 50 ~ "Intermedias (Avance entre 26 y 50% del plazo total)",
        porcentaje_avance > 50 ~ "Avanzadas (Avance > 50% del plazo total)"
      )
    )
  ...
  observe({
    responsables <- sort(unique(df_retrasadas$responsable_de_garantia))
    problemas <- sort(unique(df_retrasadas$problema_clasificado))
    
    updateSelectInput(session, "responsable_filter", 
                      choices = c("", responsables),
                      selected = "")
  ...
  output$tabla_detalle_vigentes <- renderReactable({
    ...
    tabla_resumen <- datos %>%
      group_by(responsable_de_garantia, problema_clasificado) %>%
      summarise(
        Tempranas = sum(clasificacion_avance == "Tempranas (Avance ≤ 25% del plazo total)"),
        Intermedias = sum(clasificacion_avance == "Intermedias (Avance entre 26 y 50% del plazo total)"),
        Avanzadas = sum(clasificacion_avance == "Avanzadas (Avance > 50% del plazo total)"),
        Total = n(),
        .groups = "drop"
      ) %>%
      arrange(desc(Total))
  ...
  output$grafico_evolucion_vigentes <- renderPlotly({
    ...
    evolucion <- df_hist %>%
      ...
      ungroup() %>%
      mutate(clasificacion_avance = factor(clasificacion_avance, levels = c("Avanzadas (Avance > 50% del plazo total)", "Intermedias (Avance entre 26 y 50% del plazo total)", "Tempranas (Avance ≤ 25% del plazo total)")))
    ...
    p <- plot_ly()
    colores_avance <- c("Tempranas (Avance ≤ 25% del plazo total)" = "#4CAF50", 
                        "Intermedias (Avance entre 26 y 50% del plazo total)" = "#FFC107", 
                        "Avanzadas (Avance > 50% del plazo total)" = "#E53935")
    
    for(nivel in c("Avanzadas (Avance > 50% del plazo total)", "Intermedias (Avance entre 26 y 50% del plazo total)", "Tempranas (Avance ≤ 25% del plazo total)")) {
